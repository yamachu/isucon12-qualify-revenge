      ...
      try {
        ...
        try {
          const scores = await tenantDB.all<{ title: string; score: number }[]>(
            'select ct.title as title, lps.score as score from competition ct join latest_player_score lps on lps.competition_id = ct.id where ct.tenant_id = ? order by ct.created_at ASC',
            viewer.tenantId
          )
          // for (const comp of competitions) {
  ...
  wrap(async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        await adminDB.execute<OkPacket>(
          'INSERT IGNORE INTO min_visit_history (player_id, tenant_id, competition_id, created_at) VALUES (?, ?, ?, ?)',
